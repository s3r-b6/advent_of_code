...
fn get_scenic_score(tree_grid: &Vec<Vec<u8>>) -> u32 {
    ...
    for row in 0..GRID_SIZE {
        for col in 0..GRID_SIZE {
            ...
            for upper_rows in (0..row).rev() {
                let tree_above = tree_grid[upper_rows][col];
                if previous_tree >= Some(tree_above) && previous_tree >= Some(current_tree) {
                    break;
                }
                trees_seen_above += 1;
            ...
            for left_col in (0..col).rev() {
                let tree_in_left = tree_grid[row][left_col];
                if previous_tree >= Some(tree_in_left) && previous_tree >= Some(current_tree) {
                    break;
                }
                trees_seen_left += 1;
            ...
            for below_rows in row + 1..GRID_SIZE {
                let tree_below = tree_grid[below_rows][col];
                if previous_tree >= Some(tree_below) && previous_tree >= Some(current_tree) {
                    break;
                }
                trees_seen_below += 1;
            ...
            for right_col in col + 1..GRID_SIZE {
                let tree_in_right = tree_grid[row][right_col];
                if previous_tree >= Some(tree_in_right) && previous_tree >= Some(current_tree) {
                    break;
                }
                trees_seen_right += 1;
            ...
            scenic_scores
                .push(trees_seen_above * trees_seen_below * trees_seen_left * trees_seen_right);
        }
    }
